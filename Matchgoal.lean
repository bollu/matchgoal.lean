...
partial def substitute (ctx: PatternCtx) (s : Syntax) : MatchGoalM Syntax :=
  match s with
  | `(term| #$i:ident) => do substitute ctx (← monadLift (m := TacticM) <| `(unification_var| #$i))
  | `(unification_var| #$i:ident) => do
...
def evalMatchgoal : Tactic := fun stx => -- (stx -> TacticM Unit)
  ...
  | `(tactic| matchgoal
      ...
      ⊢ $[ $g?:unification_expr ]? => $t ) => do
        IO.println g?
        let ts ← MatchGoalM.unwrap do
          let mut ctx : PatternCtx := default
          if let .some g := g? then
             ctx ← runUnificationExpr ctx (← getMainDecl).type g -- bind the goal
          substitute ctx t
        -- we have many tactics to run
        if ts.length != 1
        then logErrorAt t <| MessageData.tagged `Tactic.Matchgoal m!"expected exactly one output state, found {ts}"
        else
          for t in ts do
